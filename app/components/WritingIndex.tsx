...
import { categories, featuredPost, posts } from '../../content/posts';
...
export default function WritingIndex() {
  ...
  return (
    <>
      ...
      {/* Filter bar — sits directly beneath the sticky header. */}
      ...
      <section aria-label="Featured post" className="border-b border-ink/10 bg-raised">
        <div className="mx-auto max-w-[1280px] px-[26px] py-[52px]">
          <Link
            href={`${routes.writing}/${featuredPost.slug}`}
          ...
          >
            <div className="lc-hatch grid aspect-[4/3] place-items-center border-r border-ink/10 p-[22px]">
              <span className={`${MONO} text-center text-[10.5px]`}>FEATURED — COVER IMAGE</span>
            </div>
      ...
      <section aria-label="All posts" className="mx-auto max-w-[1280px] px-[26px] pt-14 pb-[72px]">
        <div className="grid grid-cols-[repeat(auto-fit,minmax(min(100%,300px),1fr))] gap-[22px]">
          {visible.map((post) => (
            ...
            >
              <div className="lc-hatch grid aspect-[16/10] place-items-center border-b border-ink/12 p-[18px]">
                <span className={`${MONO} text-center text-[10px]`}>COVER IMAGE</span>
              </div>
              <div className="flex min-w-0 flex-col gap-[11px] px-5 pt-[22px] pb-6">
                ...
                <div className={`${MONO} mt-1 text-[10px] tracking-[0.14em]`}>{post.date}</div>
              </div>
            </Link>
          ))}
        </div>
